...
public class Map implements Serializable {
	...
	protected void genData()
	{
		...
		int BlockCount = (int)(width * heigth * fill); // Reserve 2 for `Start` and `End`
		if  (data == null)
			data = new Cell[width * heigth];
		var preIndexes = new ArrayList<Integer>(width*heigth + 2);
		...
		//	indexes[indexes.length - 2] % width,
		//	indexes[indexes.length - 2] / width
		//);
		//System.out.printf("%d,%d:\tE\n",
		//	indexes[indexes.length - 1] % width,
		//	indexes[indexes.length - 1] / width
		//);
	}
